...
>
> 🪶 **极致轻量——零依赖，零锁定。** 整个系统就是纯 Markdown 文件。没有框架要学、没有数据库要维护、没有 Docker 要配、没有守护进程要看管。每个 skill 就是一个 `SKILL.md`，任何 LLM 都能读懂——换成 [OpenClaw](docs/OPENCLAW_ADAPTATION.md)、Cursor、Windsurf 或者你自己的 agent，工作流照样跑。**ARIS 是方法论，不是平台。** Fork 它、改写它、适配到你的技术栈。科研工作流才是价值；实现方式故意设计为可替换的。
