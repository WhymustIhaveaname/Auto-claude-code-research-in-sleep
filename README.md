...
>
> 🪶 **Radically lightweight — zero dependencies, zero lock-in.** The entire system is plain Markdown files. No framework to learn, no database to maintain, no Docker to configure, no daemon to babysit. Every skill is a single `SKILL.md` readable by any LLM — swap Claude Code for [OpenClaw](docs/OPENCLAW_ADAPTATION.md), Cursor, Windsurf, or your own agent and the workflows still work. **ARIS is a methodology, not a platform.** Fork it, rewrite it, adapt it to your stack. The research workflows are the value; the implementation is intentionally disposable.
